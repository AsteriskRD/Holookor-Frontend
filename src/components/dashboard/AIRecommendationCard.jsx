    ...
    <Card className="p-5 sm:p-6 hover:shadow-md transition-all duration-200">
      ...
      <div className="space-y-3 sm:space-y-4">
        {recommendations.map((rec) => (
          <div key={rec.id} className="p-4 sm:p-5 bg-gradient-to-br from-[var(--color-neutral-50)] to-white rounded-xl border border-[var(--color-border)] hover:shadow-sm transition-all duration-200">
            <div className="flex items-start justify-between gap-3 mb-3">
              <h4 className="text-sm sm:text-base font-bold text-[var(--color-foreground)] flex-1">{rec.title}</h4>
              <Badge variant="default" className="flex-shrink-0">{rec.difficulty}</Badge>
            </div>
            <p className="text-xs sm:text-sm text-[var(--color-muted-foreground)] mb-4 leading-relaxed">{rec.description}</p>
            <Button
              variant="primary"
              size="md"
              onClick={() => onStartQuiz(rec.id)}
              className="w-full shadow-sm hover:shadow-md hover:bg-[#0c5b29] "
            >
              Start Quiz
            </Button>
          </div>
