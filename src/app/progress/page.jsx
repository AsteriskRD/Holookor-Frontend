...
export default function ProgressPage() {
  ...
  return (
    <div className="min-h-screen bg-[var(--background)]">
      <NavBar user={data.user} notificationCount={data.notifications.unreadCount} />
      ...
      <main className="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <header className="mb-6">
          ...
          <p className="text-sm text-[var(--color-muted-foreground)] mt-1">See your learning progress so far.</p>

          <div className="mt-6 grid grid-cols-2 sm:grid-cols-4 gap-4">
            {data.kpis.map((k) => (
              <div key={k.id} className="bg-white p-4 rounded-lg border border-[var(--color-border)] flex flex-col">
                <div className="text-sm text-[var(--color-muted-foreground)]">{k.label}</div>
                <div className="text-xl font-bold text-[var(--color-foreground)] mt-2">{k.value}</div>
              </div>
            ))}
          </div>
        </header>
